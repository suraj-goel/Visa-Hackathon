    ...
    if search_option=='initial':
        ...
        if len(product_tags)>1:
            for i in product_tags:
                cur.execute("SELECT * from Location,Merchant,Product WHERE Category LIKE %s and Product.MerchantID <> %s and Location.MerchantID=Merchant.MerchantID and Product.Sell=1", ( i,merchantid))
                x=list(cur.fetchall())
                data.extend(x)
